/**
 * After a user has selected a playlist, they are taken to the
 * playlist splitter, where they can choose what genres they
 * want to create playlists out of
 */
...
export default function PlaylistSplitter() {
  ...
  function formatTracks(playlistInfo) {
    ...
    onlyTracks.forEach((track) => {
      const trackObject = new Track(
        ...
        track.album?.images[0]?.url || noImage
      ); // default to `noImage` if no image url is available
  ...
  useEffect(() => {
    ...
    const manipulateData = async () => {
      ...
      //removes outliars
      const genreCounterWithNoOutliars = eliminateOutliars(
        genreCounterWithOutliars
      );
      ...
      const genresWithTracks = addSongsToGenreCount(
        formatedTrackWithGenres,
        sortedGenreCount
      );
  ...
  return isLoading ? (
    ...
    <div className="playlist-splitter">
      ...
      <h1 className="playlist-instructions">
        Select the playlists you want to make
      </h1>

      <div className="buttons">
        <div className="genre-playlists-buttons">{genrePlaylists}</div>
